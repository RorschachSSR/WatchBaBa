...
for chp = 3%1:3
    ...
    if chp == 2
        if lvl == 4
            P = P_all(P_all.level4_withHot == ver, :);
        end
        if lvl == 5
            P = P_all(P_all.level5_withHot == ver, :);
        end
    elseif chp == 3
        if lvl == 2
            if ver == 0
                P = P_all(bonus2_noBagIsPush, :);
            elseif ver == 1
                P = P_all(bonus2_noMelt, :);
            end
        end
        if lvl == 3
            if ver == 0
                P = P_all(bonus3_noBag & ~ismember(P_all.SubNo, find(excludedPforBonus3)), :);
            elseif ver == 1
                P = P_all(bonus3_noPush & ~ismember(P_all.SubNo, find(excludedPforBonus3)), :);
            end
        end
    else
        P = P_all;
